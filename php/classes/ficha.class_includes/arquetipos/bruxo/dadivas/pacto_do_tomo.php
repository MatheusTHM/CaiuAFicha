<?php
switch (true) {
	case ($this->arq_bruxoP == 'Pacto do Tomo' and $this->nivel >2 && $this->nivel <21):
		print('<h3>Pacto do Tomo<h3/>');
	...
	$consulta = array(
		...
		mysqli_query($conexao, "select hab_class_nome, hab_class_desc from habilidade_classe where hab_class_cod=50")
		);
		
		$hab_arq_class_nome = null;
		$hab_arq_class_cod = array(50);
		array_push($hab_class_cod,50);
