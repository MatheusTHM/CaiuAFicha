<?php
switch (true) {
	case ($this->arq_bruxoP == 'Pacto da Corrente' and $this->nivel >2 && $this->nivel <21):
		print('<h3>Pacto da Corrente<h3/>');
	...
	$consulta = array(
		...
		mysqli_query($conexao, "select hab_class_nome, hab_class_desc from habilidade_classe where hab_class_cod=48")
		);
		
		$hab_arq_class_nome = null;
		$hab_arq_class_cod = array(48);
		array_push($hab_class_cod,48);
		foreach ($hab_arq_class_cod as $hab_cod){
			...
			$hab_arq_class_nome = (str_replace("$this->arq_bruxoP - ", "", $hab_arq_class_nome));
